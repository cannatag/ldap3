...
from ..operation.extended import (
    extended_request_to_dict,
    extended_response_to_dict,
    intermediate_response_to_dict,
...
class BaseStrategy(object):
    ...
    def __init__(self, ldap_connection):
        ...
        self.referral_cache = {}
        self.thread_safe = (
            False  # Indicates that connection can be used in a multithread application
        )
    ...
    def open(self, reset_usage=True, read_server_info=True):
        ...
        if self.connection.lazy and not self.connection._executing_deferred:
            ...
            if (
                not self.connection.closed and not self.connection._executing_deferred
            ...
            if self.connection.usage:
                if (
                    reset_usage
                    or not self.connection._usage.initial_connection_start_time
                ):
                    self.connection._usage.start()

            if self.connection.server_pool:
                new_server = self.connection.server_pool.get_server(
                    self.connection
                )  # get a server from the server_pool if available
                ...
                for candidate_address in self.connection.server.candidate_addresses():
                    try:
                        if log_enabled(BASIC):
                            log(
                                BASIC,
                                "try to open candidate address %s",
                                candidate_address[:-2],
                            )
                        self._open_socket(
                            candidate_address,
                            self.connection.server.ssl,
                            unix_socket=self.connection.server.ipc,
                        )
                        self.connection.server.current_address = candidate_address
                        self.connection.server.update_availability(
                        ...
                        exception_history.append(
                            (type(e)(str(e)), candidate_address[4])
                        )
                    ...
                    if len(exception_history) == 1:  # only one exception, reraise
                        if log_enabled(ERROR):
                            log(
                                ERROR,
                                "<%s> for <%s>",
                                str(exception_history[0][0])
                                + " "
                                + str((exception_history[0][1])),
                                self.connection,
                            )
                        ...
                        if log_enabled(ERROR):
                            log(
                                ERROR, "unable to open socket for <%s>", self.connection
                            )
    ...
    def close(self):
        ...
        if (
            self.connection.lazy
            and not self.connection._executing_deferred
    ...
    def _open_socket(self, address, use_ssl=False, unix_socket=False):
        ...
        try:
            ...
            raise communication_exception_factory(LDAPSocketOpenError, type(e)(str(e)))(
        ...
        if unix_socket_available and self.connection.socket.family != socket.AF_UNIX:
            ...
            for source_port in self.connection.source_port_list:
                try:
                    self.connection.socket.bind(
                        (self.connection.source_address, source_port)
                    )
                    ...
                    if log_enabled(NETWORK):
                        log(
                            NETWORK,
                            "Unable to bind to local address <%s> with source port <%s> due to <%s>",
                            self.connection.source_address,
                            source_port,
                            bind_ex,
                        )
            if not socket_bind_succeeded:
                self.connection.last_error = (
                    "socket connection error while locally binding: "
                    + str(last_bind_exc)
                )
                if log_enabled(ERROR):
                    log(
                        ERROR,
                        "Unable to locally bind to local address <%s> with any of the source ports <%s> for connection <%s due to <%s>",
                        self.connection.source_address,
                        self.connection.source_port_list,
                        self.connection,
                        last_bind_exc,
                    )
                raise communication_exception_factory(
                    LDAPSocketOpenError, type(last_bind_exc)(str(last_bind_exc))
                )(last_bind_exc)

        try:  # set socket timeout for opening connection
            if self.connection.server.connect_timeout:
                self.connection.socket.settimeout(
                    self.connection.server.connect_timeout
                )
            self.connection.socket.connect(address[4])
        except socket.error as e:
            self.connection.last_error = (
                "socket connection error while opening: " + str(e)
            )
            if log_enabled(ERROR):
                log(ERROR, "<%s> for <%s>", self.connection.last_error, self.connection)
            # raise communication_exception_factory(LDAPSocketOpenError, exc)(self.connection.last_error)
        ...
        if self.connection.receive_timeout is not None:
            try:  # set receive timeout for the connection socket
                ...
                if system().lower() == "windows":
                    self.connection.socket.setsockopt(
                        socket.SOL_SOCKET,
                        socket.SO_RCVTIMEO,
                ...
                self.connection.last_error = (
                    "unable to set receive timeout for socket connection: " + str(e)
                )
                ...
                raise communication_exception_factory(
                    LDAPSocketOpenError, type(e)(str(e))
                )(self.connection.last_error)

        if use_ssl:
            try:
                self.connection.server.tls.wrap_socket(
                    self.connection, do_handshake=True
                )
                if self.connection.usage:
    ...
    def send(self, message_type, request, controls=None):
        ...
        if self.connection.listening:
            if self.connection.sasl_in_progress and message_type not in [
                "bindRequest"
            ...
            self.connection.request = BaseStrategy.decode_request(
                message_type, request, controls
            )
        ...
        if self._outstanding and message_id in self._outstanding:
            ...
            if not responses:
                if log_enabled(ERROR):
                    log(
                        ERROR,
                        "socket timeout, no response from server for <%s>",
                        self.connection,
                    )
            ...
            if responses == SESSION_TERMINATED_BY_SERVER:
                ...
                responses == TRANSACTION_ERROR
            ):  # Novell LDAP Transaction unsolicited notification
            ...
            if responses[-2]["result"] == RESULT_REFERRAL:
                ...
                if self.connection.auto_referrals:
                    ref_response, ref_result = self.do_operation_on_referral(
                        self._outstanding[message_id], responses[-2]["referrals"]
                    )
            ...
            if (
                self.connection.auto_range
                and not hasattr(self, "_auto_range_searching")
                and any(
                    (
                        True
                        for resp in response
                        if "raw_attributes" in resp
                        for name in resp["raw_attributes"]
                        if ";range=" in name
                    )
                )
            ):
                ...
                if self.do_search_on_auto_range(
                    self._outstanding[message_id], response
                ):
                    for resp in temp_response:
                        if resp["type"] == "searchResEntry":
                            keys = [
                                key
                                for key in resp["raw_attributes"]
                                if ";range=" in key
                            ]
                            for key in keys:
            ...
            if self.connection.empty_attributes:
                for entry in response:
                    if entry["type"] == "searchResEntry":
                        for attribute_type in self._outstanding[message_id][
                            "attributes"
                        ]:
                            if attribute_type not in entry[
                                "raw_attributes"
                                ...
                                if log_enabled(PROTOCOL):
                                    log(
                                        PROTOCOL,
                                        "attribute set to empty list for missing attribute <%s> in <%s>",
                                        attribute_type,
                                        self,
                                    )
            ...
            if log_enabled(ERROR):
                log(
                    ERROR,
                    "message id not in outstanding queue for <%s>",
                    self.connection,
                )
    ...
    def compute_ldap_message_size(data):
        ...
        if len(data) > 2:
            if (
                data[1] <= 127
            ):  # BER definite length - short form. Highest bit of byte 1 is 0, message length is in the last 7 bits - Value can be up to 127 bytes long
                ret_value = data[1] + 2
    ...
    def decode_response(self, ldap_message):
        ...
        controls = (
            ldap_message["controls"] if ldap_message["controls"].hasValue() else None
        )
        if message_type == "bindResponse":
            if not bytes(component["matchedDN"]).startswith(
                b"NTLM"
            ):  # patch for microsoft ntlm authentication
                result = bind_response_to_dict(component)
            else:
                result = sicily_bind_response_to_dict(component)
        elif message_type == "searchResEntry":
            result = search_result_entry_response_to_dict(
                component,
                self.connection.server.schema,
                self.connection.server.custom_formatter,
    ...
    def decode_response_fast(self, ldap_message):
        ...
        if ldap_message["protocolOp"] == 1:  # bindResponse
            if not ldap_message["payload"][1][3].startswith(
                b"NTLM"
            ):  # patch for microsoft ntlm authentication
            ...
            result = search_result_entry_response_to_dict_fast(
                ldap_message["payload"],
                self.connection.server.schema,
                self.connection.server.custom_formatter,
                self.connection.check_names,
            )
            ...
            result = search_result_reference_response_to_dict_fast(
                ldap_message["payload"]
            )
    ...
    def decode_control(control):
        ...
        if (
        ...
        ):  # simple paged search as per RFC2696
            control_resp, unprocessed = decoder.decode(
                control_value, asn1Spec=RealSearchControlValue()
            )
            control_value = dict()
            ...
            control_resp, unprocessed = decoder.decode(
                control_value, asn1Spec=DirSyncControlResponseValue()
            )
            control_value = dict()
            control_value["more_results"] = bool(
                control_resp["MoreResults"]
            )  # more_result if nonzero
            control_value["cookie"] = bytes(control_resp["CookieServer"])
        elif (
            control_type == "1.3.6.1.1.13.1" or control_type == "1.3.6.1.1.13.2"
        ):  # Pre-Read control, Post-Read Control as per RFC 4527
            control_resp, unprocessed = decoder.decode(
                control_value, asn1Spec=SearchResultEntry()
            )
            control_value = dict()
        ...
        return control_type, {
            "description": Oids.get(control_type, ""),
    ...
    def decode_control_fast(control, from_server=True):
        ...
        for r in control[1:]:
            if r[2] == 4:  # controlValue
                ...
                )  # criticality (booleand default to False)
        if (
            control_type == "1.2.840.113556.1.4.319"
        ):  # simple paged search as per RFC2696
            ...
            control_resp = decode_sequence(control_value, 0, len(control_value))
            control_value = dict()
            control_value["more_results"] = (
                True if control_resp[0][3][0][3] else False
            )  # more_result if nonzero
    ...
    def do_next_range_search(self, request, response, attr_name):
        ...
        while not done:
            ...
            response["raw_attributes"][attr_type] += current_response["raw_attributes"][
                attr_name
            ]
            ...
            if high_range != "*":
                if log_enabled(PROTOCOL):
                    log(
                        PROTOCOL,
                        "performing next search on auto-range <%s> via <%s>",
                        str(int(high_range) + 1),
                        self.connection,
                    )
                requested_range = (
                    attr_type + ";range=" + str(int(high_range) + 1) + "-*"
                )
                result = self.connection.search(
                    search_base=response["dn"],
                ...
                if not done:
                    if (
                        requested_range in current_response["raw_attributes"]
                        and len(current_response["raw_attributes"][requested_range])
                        == 0
                    ):
                    ...
                    attr_name = list(
                        filter(
                            lambda a: ";range=" in a,
                            current_response["raw_attributes"].keys(),
                        )
                    )[0]
                    continue
            done = True
    ...
    def do_search_on_auto_range(self, request, response):
        for resp in [r for r in response if r["type"] == "searchResEntry"]:
            for attr_name in list(
                resp["raw_attributes"].keys()
            ):  # generate list to avoid changing of dict size error
                if ";range=" in attr_name:
                    attr_type, _, range_values = attr_name.partition(";range=")
                    if range_values in (
                        "1-1",
                        "0-0",
                    ):  # DirSync returns these values for adding and removing members
                        return False
                    if (
                        attr_type not in resp["raw_attributes"]
                        or resp["raw_attributes"][attr_type] is None
                    ):
                        resp["raw_attributes"][attr_type] = list()
                    if (
                        attr_type not in resp["attributes"]
                        or resp["attributes"][attr_type] is None
                    ):
    ...
    def create_referral_connection(self, referrals):
        ...
        if valid_referral_list:
            preferred_referral_list = [
                referral
                for referral in valid_referral_list
                if referral["ssl"] == self.connection.server.ssl
            ]
            selected_referral = (
                choice(preferred_referral_list)
                if preferred_referral_list
                else choice(valid_referral_list)
            )

            cachekey = (
                selected_referral["host"],
                selected_referral["port"] or self.connection.server.port,
                selected_referral["ssl"],
            )
            if self.connection.use_referral_cache and cachekey in self.referral_cache:
                ...
                referral_server = Server(
                    host=selected_referral["host"],
                    ...
                    tls=Tls(
                        local_private_key_file=self.connection.server.tls.private_key_file,
                        ...
                        ca_certs_file=self.connection.server.tls.ca_certs_file,
                    )
                    if selected_referral["ssl"]
                    else None,
                )

                from ..core.connection import Connection

                referral_connection = Connection(
                    server=referral_server,
                    user=self.connection.user
                    if not selected_referral["anonymousBindOnly"]
                    else None,
                ...
                if (
                    self.connection.tls_started and not referral_server.ssl
                ):  # if the original server was in start_tls mode and the referral server is not in ssl then start_tls on the referral connection
                    if not referral_connection.start_tls():
                        error = "start_tls in referral not successful" + (
                            " - " + referral_connection.last_error
                            if referral_connection.last_error
                            else ""
                        )
                        if log_enabled(ERROR):
    ...
    def do_operation_on_referral(self, request, referrals):
        ...
        (
            ...
            referral_connection,
            cachekey,
        ) = self.create_referral_connection(referrals)
        if selected_referral:
            if request["type"] == "searchRequest":
                referral_connection.search(
                    selected_referral["base"] or request["base"],
                ...
                referral_connection.add(
                    selected_referral["base"] or request["entry"],
                    None,
                    request["attributes"],
                    controls=request["controls"],
                )
            elif request["type"] == "compareRequest":
                referral_connection.compare(
                    selected_referral["base"] or request["entry"],
                    request["attribute"],
                    request["value"],
                    controls=request["controls"],
                ...
                referral_connection.delete(
                    selected_referral["base"] or request["entry"],
                    controls=request["controls"],
                )
            elif request["type"] == "extendedReq":
                referral_connection.extended(
                    request["name"],
                    request["value"],
                    controls=request["controls"],
                    no_encode=True,
        ...
        try:
            ...
            if (
                ...
                signature = bytes.fromhex(
                    md5_hmac(kic, int(sec_num).to_bytes(4, "big") + encoded_message)[
                        0:20
                    ]
                )
                encoded_message = (
                    int(len(encoded_message) + 4 + 2 + 10).to_bytes(4, "big")
                    + encoded_message
                    + signature
