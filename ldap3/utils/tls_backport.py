...
import re
from ..utils.log import log, log_enabled, ERROR, BASIC, PROTOCOL, PROTOCOL_CHATTY
...
def _dnsname_match_backport(dn, hostname, max_wildcards=1):
    ...
    """
    if log_enabled(PROTOCOL_CHATTY):
        log(PROTOCOL_CHATTY, "matching dn %s with hostname %s", dn, hostname)
    pats = []
    if not dn:
        return False
