...
import re
from ..utils.log import log, log_enabled, VERBOSITY_ERROR, VERBOSITY_BASIC, VERBOSITY_PROTOCOL, VERBOSITY_PROTOCOL_CHATTY
...
def _dnsname_match_backport(dn, hostname, max_wildcards=1):
    ...
    """
    if log_enabled(VERBOSITY_PROTOCOL_CHATTY):
        log(VERBOSITY_PROTOCOL_CHATTY, "matching dn %s with hostname %s", dn, hostname)
    pats = []
    if not dn:
        return False
